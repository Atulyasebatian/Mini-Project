...
// FIX: Added 'useNavigate', 'Link', and 'useLocation' to the import
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { Spinner } from 'react-bootstrap';
import  jwtDecode  from 'jwt-decode';
// FIX: Import the reusable components
import PassengerHeader from '../../components/PassengerHeader';
import PassengerBottomNav from '../../components/PassengerBottomNav';
import '../../styles/history.css';

function HistoryPage() {
  // FIX: Initialize the navigate hook
  const navigate = useNavigate();
  // FIX: Define the 'user' state and its setter
    ...
    const fetchData = async () => {
      ...
      try {
        const [tripRes, reportRes] = await Promise.all([
          axios.get("http://localhost:3000/api/trips/my-trips", { headers: { Authorization: `Bearer ${token}` } }),
          axios.get("http://localhost:3000/api/reports/my-reports", { headers: { Authorization: `Bearer ${token}` } })
        ]);
